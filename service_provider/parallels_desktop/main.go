...
func (s *ParallelsService) GetVms() ([]models.ParallelsVM, error) {
	...
	users, err := system.Get().GetSystemUsers()
	if err != nil {
		return nil, err
	}
	...
	for _, user := range users {
		logger.Info("Getting VMs for user: " + user.Username)
...
func (s *ParallelsService) GetFilteredVm(filter string) ([]models.ParallelsVM, error) {
	...
	if len(vms) == 0 {
		return nil, errors.ErrNoVirtualMachinesFound()
	}

	if !hasProperty(vms[0], filterParts[0]) {
		return nil, errors.ErrInvalidFilterProperty()
	}
...
func (s *ParallelsService) RegisterVm(r models.RegisterVirtualMachineRequest) error {
	if r.Uuid != "" {
		vm, err := s.findVm(r.Uuid)
		if err != nil {
			return err
		}
		if vm != nil {
			return fmt.Errorf("VM with UUID %s already exists", r.Uuid)
		}
	}
	if r.MachineName != "" {
		vm, err := s.findVm(r.MachineName)
		if err != nil {
			return err
		}
		if vm != nil {
...
func (s *ParallelsService) UnregisterVm(r models.UnregisterVirtualMachineRequest) error {
	vm, err := s.findVm(r.ID)
	if err != nil {
		return err
	}
	if vm == nil {
...
func (s *ParallelsService) RenameVm(r models.RenameVirtualMachineRequest) error {
	vm, err := s.findVm(r.GetId())
	if err != nil {
		return err
	}
	if vm == nil {
...
func (s *ParallelsService) ConfigureVm(id string, setOperations *models.VirtualMachineConfigRequest) error {
	vm, err := s.findVm(id)
	if err != nil {
		return err
	}
	...
	for _, op := range setOperations.Operations {
		...
		case "boot-order":
			common.Logger.Info("Setting boot order property %s to %s", op.Operation, op.Value)
			if err := s.SetVmBootOperation(vm, op); err != nil {
				op.Error = err
			}
		case "efi-secure-boot":
			common.Logger.Info("Setting boot efi secure boot property %s to %s", op.Operation, op.Value)
			if err := s.SetVmBootOperation(vm, op); err != nil {
				op.Error = err
			}
		case "select-boot-device":
			common.Logger.Info("Setting select boot device property %s to %s", op.Operation, op.Value)
			if err := s.SetVmBootOperation(vm, op); err != nil {
				op.Error = err
			}
		case "external-boot-device":
			common.Logger.Info("Setting external boot device property %s to %s", op.Operation, op.Value)
			if err := s.SetVmBootOperation(vm, op); err != nil {
				op.Error = err
			}
		case "network":
		...
		case "shared_folder":
			...
			if err := s.SetVmSharedFolderOperation(vm, op); err != nil {
				op.Error = err
			}
		case "rosetta":
...
func (s *ParallelsService) SetVmMachineOperation(vm *models.ParallelsVM, op *models.VirtualMachineConfigRequestOperation) error {
	cmd := helpers.Command{
		Command: "sudo",
		Args:    make([]string, 0),
	}
...
func (s *ParallelsService) SetVmBootOperation(vm *models.ParallelsVM, op *models.VirtualMachineConfigRequestOperation) error {
	cmd := helpers.Command{
		Command: "sudo",
		Args:    make([]string, 0),
	}
...
func (s *ParallelsService) SetVmSharedFolderOperation(vm *models.ParallelsVM, op *models.VirtualMachineConfigRequestOperation) error {
	cmd := helpers.Command{
		Command: "sudo",
		Args:    make([]string, 0),
	}
...
func (s *ParallelsService) SetVmCpu(vm *models.ParallelsVM, op *models.VirtualMachineConfigRequestOperation) error {
	if vm.State != "stopped" {
		return errors.New("VM is not stopped")
	}
	cmd := "sudo"
...
func (s *ParallelsService) SetVmMemory(vm *models.ParallelsVM, op *models.VirtualMachineConfigRequestOperation) error {
	if vm.State != "stopped" {
		return errors.New("VM is not stopped")
	}
	cmd := "sudo"
...
func (s *ParallelsService) SetVmRosettaEmulation(vm *models.ParallelsVM, op *models.VirtualMachineConfigRequestOperation) error {
	if vm.State != "stopped" {
		return errors.New("VM is not stopped")
	}
	cmd := "sudo"
