  ...
  - path: /v1/auth/token
    ...
    example_blocks:
      - title: cURL
        language: powershell
        code_block: |
          curl --location '{{host}}/v1/auth/token' \
      ...
      - title: C#
        ...
        code_block: |
          ...
          var request = new HttpRequestMessage(HttpMethod.Post, "{{host}}/v1/auth/token");
  ...
  - path: /v1/auth/token/validate
    ...
    example_blocks:
      - title: cURL
        language: powershell
        code_block: |
          curl --location '{{host}}/v1/auth/token/validate' \
          --header 'Content-Type: application/json' \
          --data '{\n  \"key\": \"SomeKey\",\n  \"secret\": \"SomeLongSecret\"\n}'
      - title: C#
        language: csharp
        code_block: |
          var client = new HttpClient();
          var request = new HttpRequestMessage(HttpMethod.Post, "{{host}}/v1/auth/token/validate");
          request.Headers.Add("Authorization", "******");
          request.Content = new StringContent("{\n  \"key\": \"SomeKey\",\n  \"secret\": \"SomeLongSecret\"\n}", Encoding.UTF8, "application/json");
          var response = await client.SendAsync(request);
      - title: Go
        ...
        code_block: |
          ...
          func main() {
              url := "{{host}}/v1/auth/token/validate"
              method := "POST"
...
This section contains all authorization related endpoints.
