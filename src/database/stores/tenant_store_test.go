...
func TestTenantDataStoreWithSQLite(t *testing.T) {
	...
	t.Run("GetTenantByIDOrSlug_ByID", func(t *testing.T) {
		...
		createdTenant, diag := store.CreateTenant(ctx, &models.Tenant{
	...
	t.Run("GetTenants", func(t *testing.T) {
		...
		tenant1, diag := store.CreateTenant(ctx, &models.Tenant{
			Name:   "Tenant 1",
			Domain: "tenant1.com",
		})
		require.False(t, diag.HasErrors())
		...
		tenant2, diag := store.CreateTenant(ctx, &models.Tenant{
			Name:   "Tenant 2",
			Domain: "tenant2.com",
		})
		require.False(t, diag.HasErrors())
	...
	t.Run("UpdateTenant", func(t *testing.T) {
		...
		err := store.GetDB().Model(&models.Tenant{}).Where("id = ?", createdTenant.ID).Updates(map[string]interface{}{
