...
type IpBanStoreInterface interface {
	interfaces.Store
	GetIpBan(ctx basecontext.BaseContext, ip string) (*models.IpBan, *apperrors.Diagnostics)

	CreateIpBan(ctx basecontext.BaseContext, ipBan *models.IpBan) (*models.IpBan, *apperrors.Diagnostics)
	RevokeIpBan(ctx basecontext.BaseContext, ip string) *apperrors.Diagnostics
	GetActiveBans(ctx basecontext.BaseContext) ([]models.IpBan, *apperrors.Diagnostics)
	GetDB() *gorm.DB
}
...
func InitializeIpBanDataStore(db *gorm.DB) (IpBanStoreInterface, *apperrors.Diagnostics) {
	...
	if err != nil {
		diag := apperrors.NewDiagnostics("initialize_ip_ban_data_store")
		diag.AddError("failed_to_initialize_ip_ban_store", err.Error(), "ip_ban_data_store", nil)
		return nil, diag
	}
	return ipBanDataStoreInstance, nil
}
...
func (s *IpBanDataStore) Migrate() error {
	if err := s.GetDB().AutoMigrate(&models.IpBan{}); err != nil {
		return fmt.Errorf("failed to migrate ip_bans table: %v", err)
	}
	return nil
}

func (s *IpBanDataStore) GetIpBan(ctx basecontext.BaseContext, ip string) (*models.IpBan, *apperrors.Diagnostics) {
	diag := apperrors.NewDiagnostics("get_ip_ban")
	var ipBan models.IpBan
...
func (s *IpBanDataStore) CreateIpBan(ctx basecontext.BaseContext, ipBan *models.IpBan) (*models.IpBan, *apperrors.Diagnostics) {
	diag := apperrors.NewDiagnostics("create_ip_ban")
	if ipBan.ID == "" {
	...
	err := s.GetDB().WithContext(ctx.Context()).Where("ip = ?", ipBan.IP).First(&existing).Error
	if err == nil {
		// Update existing
		...
		if err := s.GetDB().WithContext(ctx.Context()).Save(&existing).Error; err != nil {
	...
	if err := s.GetDB().WithContext(ctx.Context()).Create(ipBan).Error; err != nil {
...
func (s *IpBanDataStore) RevokeIpBan(ctx basecontext.BaseContext, ip string) *apperrors.Diagnostics {
	diag := apperrors.NewDiagnostics("revoke_ip_ban")
	...
	err := s.GetDB().WithContext(ctx.Context()).Model(&models.IpBan{}).
		Where("ip = ?", ip).
...
func (s *IpBanDataStore) GetActiveBans(ctx basecontext.BaseContext) ([]models.IpBan, *apperrors.Diagnostics) {
	diag := apperrors.NewDiagnostics("get_active_bans")
