...
type MessageDataStoreInterface interface {
	...
	UpdateMessageStatus(ctx context.Context, messageID string, status models.MessageStatus, errorMsg string) error
	...
	GetStuckRetryingMessages(ctx context.Context, maxRetryAge time.Duration) ([]*models.Message, error)
	...
	CreateMessageEvent(ctx context.Context, event *models.MessageEvent) error
	CreateWorker(ctx context.Context, worker *models.Worker) error
	DeleteWorker(ctx context.Context, workerName string) error
	GetWorkerByName(ctx context.Context, workerName string) (*models.Worker, error)
	GetAllWorkers(ctx context.Context) ([]*models.Worker, error)
...
func InitializeMessageDataStore(db *gorm.DB) (MessageDataStoreInterface, *apperrors.Diagnostics) {
	...
	if err != nil {
		diag := apperrors.NewDiagnostics("initialize_message_data_store")
		diag.AddError("failed_to_initialize_message_store", err.Error(), "message_data_store", nil)
		return nil, diag
	}
	return messageDataStoreInstance, nil
}
...
func (s *MessageDataStore) Migrate() error {
	if err := s.GetDB().AutoMigrate(&models.Message{}); err != nil {
		return fmt.Errorf("failed to migrate message table: %w", err)
	}
	if err := s.GetDB().AutoMigrate(&models.MessageEvent{}); err != nil {
		return fmt.Errorf("failed to migrate message event table: %w", err)
	}
	if err := s.GetDB().AutoMigrate(&models.Worker{}); err != nil {
...
func (s *MessageDataStore) CreateMessage(ctx context.Context, message *models.Message) error {
...
func (s *MessageDataStore) UpdateMessageStatus(ctx context.Context, messageID string, status models.MessageStatus, errorMsg string) error {
	...
	if status == models.MessageStatusCompleted || status == models.MessageStatusFailed {
...
func (s *MessageDataStore) MarkMessageProcessing(ctx context.Context, messageID string) error {
	return s.UpdateMessageStatus(ctx, messageID, models.MessageStatusProcessing, "")
}
...
func (s *MessageDataStore) CompleteMessage(ctx context.Context, messageID string) error {
	return s.UpdateMessageStatus(ctx, messageID, models.MessageStatusCompleted, "")
}
...
func (s *MessageDataStore) GetMessageStats(ctx context.Context) (*models.MessageStats, error) {
...
func (s *MessageDataStore) GetStuckRetryingMessages(ctx context.Context, maxRetryAge time.Duration) ([]*models.Message, error) {
...
func (s *MessageDataStore) PerformStartupRecovery(ctx context.Context) error {
	...
	err = s.GetDB().WithContext(ctx).
		...
		Where("status = ? AND retry_count >= max_retries", models.MessageStatusRetrying).
		Updates(map[string]interface{}{
			"status":     models.MessageStatusAbandoned,
			"updated_at": time.Now(),
		}).Error
	if err != nil {
