...
type WebAuthnStoreInterface interface {
	interfaces.Store
	GetCredentialsByUser(ctx basecontext.BaseContext, userID string) ([]models.WebAuthnCredential, *apperrors.Diagnostics)

	GetCredentialByCredentialID(ctx basecontext.BaseContext, credentialID []byte) (*models.WebAuthnCredential, *apperrors.Diagnostics)
	SaveCredential(ctx basecontext.BaseContext, cred *models.WebAuthnCredential) *apperrors.Diagnostics
	UpdateCredential(ctx basecontext.BaseContext, cred *models.WebAuthnCredential) *apperrors.Diagnostics
}
...
func InitializeWebAuthnDataStore(db *gorm.DB) (WebAuthnStoreInterface, *apperrors.Diagnostics) {
	...
	if err != nil {
		diag := apperrors.NewDiagnostics("initialize_webauthn_data_store")
		diag.AddError("failed_to_initialize_webauthn_store", err.Error(), "webauthn_data_store", nil)
		return nil, diag
	}
	return webAuthnDataStoreInstance, nil
}
...
func (s *WebAuthnDataStore) Migrate() error {
	if err := s.GetDB().AutoMigrate(&models.WebAuthnCredential{}); err != nil {
		return fmt.Errorf("failed to migrate webauthn credential table: %v", err)
	}
	return nil
}

func (s *WebAuthnDataStore) GetCredentialsByUser(ctx basecontext.BaseContext, userID string) ([]models.WebAuthnCredential, *apperrors.Diagnostics) {
	diag := apperrors.NewDiagnostics("get_credentials_by_user")
	var credentials []models.WebAuthnCredential
	if err := s.GetDB().WithContext(ctx.Context()).Where("user_id = ?", userID).Find(&credentials).Error; err != nil {
...
func (s *WebAuthnDataStore) GetCredentialByCredentialID(ctx basecontext.BaseContext, credentialID []byte) (*models.WebAuthnCredential, *apperrors.Diagnostics) {
	diag := apperrors.NewDiagnostics("get_credential_by_id")
	var cred models.WebAuthnCredential
	result := s.GetDB().WithContext(ctx.Context()).Where("credential_id = ?", credentialID).First(&cred)
	if result.Error != nil {
...
func (s *WebAuthnDataStore) SaveCredential(ctx basecontext.BaseContext, cred *models.WebAuthnCredential) *apperrors.Diagnostics {
	diag := apperrors.NewDiagnostics("save_credential")
	if cred.CreatedAt.IsZero() {
		cred.CreatedAt = time.Now()
	}
	cred.UpdatedAt = time.Now()

	if err := s.GetDB().WithContext(ctx.Context()).Create(cred).Error; err != nil {
...
func (s *WebAuthnDataStore) UpdateCredential(ctx basecontext.BaseContext, cred *models.WebAuthnCredential) *apperrors.Diagnostics {
	diag := apperrors.NewDiagnostics("update_credential")
	cred.UpdatedAt = time.Now()
	// Usually WebAuthn updates are for SignCount
	if err := s.GetDB().WithContext(ctx.Context()).Save(cred).Error; err != nil {
