...
type ClaimDataStoreInterface interface {
	interfaces.Store
	GetClaims(ctx basecontext.BaseContext, tenantID string) ([]entities.Claim, *apperrors.Diagnostics)

	GetClaimsByQuery(ctx basecontext.BaseContext, tenantID string, queryBuilder *filters.QueryBuilder) (*filters.QueryBuilderResponse[entities.Claim], *apperrors.Diagnostics)
	GetClaimBySlugOrID(ctx basecontext.BaseContext, tenantID string, slugOrID string) (*entities.Claim, *apperrors.Diagnostics)
	GetClaimUsers(ctx basecontext.BaseContext, tenantID string, claimID string) ([]entities.User, *apperrors.Diagnostics)
	GetClaimUsersByQuery(ctx basecontext.BaseContext, tenantID string, claimID string, queryBuilder *filters.QueryBuilder) (*filters.QueryBuilderResponse[entities.User], *apperrors.Diagnostics)
	CreateClaim(ctx basecontext.BaseContext, tenantID string, claim *entities.Claim) (*entities.Claim, *apperrors.Diagnostics)
	UpdateClaim(ctx basecontext.BaseContext, tenantID string, claim *entities.Claim) *apperrors.Diagnostics
	DeleteClaim(ctx basecontext.BaseContext, tenantID string, id string) *apperrors.Diagnostics
	GetClaimsByLevel(ctx basecontext.BaseContext, tenantID string, level models.SecurityLevel) ([]entities.Claim, *apperrors.Diagnostics)
	...
	GetClaimApiKeys(ctx basecontext.BaseContext, tenantID string, claimID string) ([]entities.ApiKey, *apperrors.Diagnostics)
	GetClaimApiKeysByQuery(ctx basecontext.BaseContext, tenantID string, claimID string, queryBuilder *filters.QueryBuilder) (*filters.QueryBuilderResponse[entities.ApiKey], *apperrors.Diagnostics)
	...
	GetClaimRoles(ctx basecontext.BaseContext, tenantID string, claimID string) ([]entities.Role, *apperrors.Diagnostics)
	GetClaimRolesByQuery(ctx basecontext.BaseContext, tenantID string, claimID string, queryBuilder *filters.QueryBuilder) (*filters.QueryBuilderResponse[entities.Role], *apperrors.Diagnostics)
...
func InitializeClaimDataStore(db *gorm.DB) (ClaimDataStoreInterface, *apperrors.Diagnostics) {
	...
	if err != nil {
		diag := errors.NewDiagnostics("initialize_claim_data_store")
		diag.AddError("failed_to_initialize_claim_store", err.Error(), "claim_data_store", err)
		return nil, diag
	}
	return claimDataStoreInstance, nil
}
...
func (s *ClaimDataStore) GetClaims(ctx basecontext.BaseContext, tenantID string) ([]entities.Claim, *apperrors.Diagnostics) {
	diag := errors.NewDiagnostics("store_get_claims")
	if tenantID == "" {
	...
	result := db.WithContext(ctx).Where("tenant_id = ?", tenantID).Find(&claims)
	if result.Error != nil {
...
func (s *ClaimDataStore) GetClaimsByQuery(ctx basecontext.BaseContext, tenantID string, queryBuilder *filters.QueryBuilder) (*filters.QueryBuilderResponse[entities.Claim], *apperrors.Diagnostics) {
	diag := errors.NewDiagnostics("store_get_claims_by_query")
	if tenantID == "" {
		diag.AddError("tenant_id_cannot_be_empty", "tenant ID cannot be empty", "claim_data_store", nil)
		return nil, diag
	}
...
func (s *ClaimDataStore) GetClaimBySlugOrID(ctx basecontext.BaseContext, tenantID string, slugOrID string) (*entities.Claim, *apperrors.Diagnostics) {
	diag := errors.NewDiagnostics("store_get_claim_by_slug_or_id")
	if tenantID == "" {
		diag.AddError("tenant_id_cannot_be_empty", "tenant ID cannot be empty", "claim_data_store", nil)
		return nil, diag
	}
...
func (s *ClaimDataStore) GetClaimUsers(ctx basecontext.BaseContext, tenantID string, claimID string) ([]entities.User, *apperrors.Diagnostics) {
	diag := errors.NewDiagnostics("store_get_claim_users")
	if tenantID == "" {
		diag.AddError("tenant_id_cannot_be_empty", "tenant ID cannot be empty", "claim_data_store", nil)
		return nil, diag
	}
...
func (s *ClaimDataStore) GetClaimUsersByQuery(ctx basecontext.BaseContext, tenantID string, claimID string, queryBuilder *filters.QueryBuilder) (*filters.QueryBuilderResponse[entities.User], *apperrors.Diagnostics) {
	diag := errors.NewDiagnostics("store_get_claim_users_by_query")
	if tenantID == "" {
		diag.AddError("tenant_id_cannot_be_empty", "tenant ID cannot be empty", "claim_data_store", nil)
		return nil, diag
	}
...
func (s *ClaimDataStore) CreateClaim(ctx basecontext.BaseContext, tenantID string, claim *entities.Claim) (*entities.Claim, *apperrors.Diagnostics) {
	diag := errors.NewDiagnostics("store_create_claim")
	if tenantID == "" {
		diag.AddError("tenant_id_cannot_be_empty", "tenant ID cannot be empty", "claim_data_store", nil)
		return nil, diag
	}
	...
	result := s.GetDB().WithContext(ctx).Create(claim)
	if result.Error != nil {
...
func (s *ClaimDataStore) UpdateClaim(ctx basecontext.BaseContext, tenantID string, claim *entities.Claim) *apperrors.Diagnostics {
	diag := errors.NewDiagnostics("store_update_claim")
	if tenantID == "" {
		diag.AddError("tenant_id_cannot_be_empty", "tenant ID cannot be empty", "claim_data_store", nil)
		return diag
	}
...
func (s *ClaimDataStore) DeleteClaim(ctx basecontext.BaseContext, tenantID string, id string) *apperrors.Diagnostics {
	diag := errors.NewDiagnostics("store_delete_claim")
	if tenantID == "" {
		diag.AddError("tenant_id_cannot_be_empty", "tenant ID cannot be empty", "claim_data_store", nil)
		return diag
	}
...
func (s *ClaimDataStore) GetClaimsByLevel(ctx basecontext.BaseContext, tenantID string, level models.SecurityLevel) ([]entities.Claim, *apperrors.Diagnostics) {
	diag := errors.NewDiagnostics("store_get_claims_by_level")
	if tenantID == "" {
		diag.AddError("tenant_id_cannot_be_empty", "tenant ID cannot be empty", "claim_data_store", nil)
		return nil, diag
	}
...
func (s *ClaimDataStore) AddClaimToUser(ctx basecontext.BaseContext, tenantID string, userID string, claimIdOrSlug string) *apperrors.Diagnostics {
	diag := errors.NewDiagnostics("store_add_claim_to_user")
	if tenantID == "" {
		diag.AddError("tenant_id_cannot_be_empty", "tenant ID cannot be empty", "claim_data_store", nil)
		return diag
	}
	...
	result = s.GetDB().WithContext(ctx).Where("user_id = ? AND claim_id = ?", user.ID, existingClaim.ID).First(&userClaims)
	if result.Error != nil {
		if !errors.Is(result.Error, gorm.ErrRecordNotFound) {
	...
	result = s.GetDB().WithContext(ctx).Create(&userClaims)
	if result.Error != nil {
...
func (s *ClaimDataStore) RemoveClaimFromUser(ctx basecontext.BaseContext, tenantID string, userID string, claimIdOrSlug string) *apperrors.Diagnostics {
	diag := errors.NewDiagnostics("store_remove_claim_from_user")
	if tenantID == "" {
		diag.AddError("tenant_id_cannot_be_empty", "tenant ID cannot be empty", "claim_data_store", nil)
		return diag
	}
	...
	result = s.GetDB().WithContext(ctx).Where("user_id = ? AND claim_id = ?", user.ID, existingClaim.ID).Delete(&userClaims)
	if result.Error != nil {
...
func (s *ClaimDataStore) GetClaimApiKeys(ctx basecontext.BaseContext, tenantID string, claimID string) ([]entities.ApiKey, *apperrors.Diagnostics) {
	diag := errors.NewDiagnostics("store_get_claim_api_keys")
	if tenantID == "" {
		diag.AddError("tenant_id_cannot_be_empty", "tenant ID cannot be empty", "claim_data_store", nil)
		return nil, diag
	}
...
func (s *ClaimDataStore) GetClaimApiKeysByQuery(ctx basecontext.BaseContext, tenantID string, claimID string, queryBuilder *filters.QueryBuilder) (*filters.QueryBuilderResponse[entities.ApiKey], *apperrors.Diagnostics) {
	diag := errors.NewDiagnostics("store_get_claim_api_keys_by_query")
	if tenantID == "" {
		diag.AddError("tenant_id_cannot_be_empty", "tenant ID cannot be empty", "claim_data_store", nil)
		return nil, diag
	}
...
func (s *ClaimDataStore) AddClaimToApiKey(ctx basecontext.BaseContext, tenantID string, claimID string, apiKeyID string) *apperrors.Diagnostics {
	diag := errors.NewDiagnostics("store_add_claim_to_api_key")
	if tenantID == "" {
		diag.AddError("tenant_id_cannot_be_empty", "tenant ID cannot be empty", "claim_data_store", nil)
		return diag
	}
	...
	result = s.GetDB().WithContext(ctx).Where("api_key_id = ? AND claim_id = ?", apiKey.ID, existingClaim.ID).First(&apiKeyClaims)
	if result.Error != nil {
		if !errors.Is(result.Error, gorm.ErrRecordNotFound) {
...
func (s *ClaimDataStore) RemoveClaimFromApiKey(ctx basecontext.BaseContext, tenantID string, claimID string, apiKeyID string) *apperrors.Diagnostics {
	diag := errors.NewDiagnostics("store_remove_claim_from_api_key")
	if tenantID == "" {
		diag.AddError("tenant_id_cannot_be_empty", "tenant ID cannot be empty", "claim_data_store", nil)
		return diag
	}
	...
	result = s.GetDB().WithContext(ctx).Where("api_key_id = ? AND claim_id = ?", apiKey.ID, existingClaim.ID).Delete(&apiKeyClaims)
	if result.Error != nil {
...
func (s *ClaimDataStore) GetClaimRolesByQuery(ctx basecontext.BaseContext, tenantID string, claimID string, queryBuilder *filters.QueryBuilder) (*filters.QueryBuilderResponse[entities.Role], *apperrors.Diagnostics) {
	diag := errors.NewDiagnostics("store_get_claim_roles_by_query")
	if tenantID == "" {
		diag.AddError("tenant_id_cannot_be_empty", "tenant ID cannot be empty", "claim_data_store", nil)
		return nil, diag
	}
...
func (s *ClaimDataStore) GetClaimRoles(ctx basecontext.BaseContext, tenantID string, claimID string) ([]entities.Role, *apperrors.Diagnostics) {
	diag := errors.NewDiagnostics("store_get_claim_roles")
	if tenantID == "" {
		diag.AddError("tenant_id_cannot_be_empty", "tenant ID cannot be empty", "claim_data_store", nil)
		return nil, diag
	}
...
func (s *ClaimDataStore) AddClaimToRole(ctx basecontext.BaseContext, tenantID string, claimID string, roleID string) *apperrors.Diagnostics {
	...
	result = s.GetDB().WithContext(ctx).Where("role_id = ? AND claim_id = ?", role.ID, existingClaim.ID).First(&roleClaims)
	if result.Error != nil {
		if !errors.Is(result.Error, gorm.ErrRecordNotFound) {
	...
	result = s.GetDB().WithContext(ctx).Create(&roleClaims)
	if result.Error != nil {
...
func (s *ClaimDataStore) RemoveClaimFromRole(ctx basecontext.BaseContext, tenantID string, claimID string, roleID string) *apperrors.Diagnostics {
	...
	result = s.GetDB().WithContext(ctx).Where("role_id = ? AND claim_id = ?", role.ID, existingClaim.ID).Delete(&roleClaims)
	if result.Error != nil {
