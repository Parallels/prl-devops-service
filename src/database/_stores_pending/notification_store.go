...
type NotificationDataStoreInterface interface {
	interfaces.Store
	CreateNotification(ctx basecontext.BaseContext, tenantID string, notification *entities.Notification) (*entities.Notification, *apperrors.Diagnostics)

	GetNotifications(ctx basecontext.BaseContext, tenantID string, userID string, filterObj *filters.QueryBuilder) (*filters.QueryBuilderResponse[entities.Notification], *apperrors.Diagnostics)
	...
	CreateChannel(ctx basecontext.BaseContext, tenantID string, channel *entities.NotificationChannel) (*entities.NotificationChannel, *apperrors.Diagnostics)
	GetChannelByName(ctx basecontext.BaseContext, tenantID string, name string) (*entities.NotificationChannel, *apperrors.Diagnostics)
	GetChannels(ctx basecontext.BaseContext, tenantID string) ([]entities.NotificationChannel, *apperrors.Diagnostics)
	AddConfigToChannel(ctx basecontext.BaseContext, tenantID string, config *entities.NotificationChannelConfig) (*entities.NotificationChannelConfig, *apperrors.Diagnostics)
	GetConfigsForChannel(ctx basecontext.BaseContext, tenantID string, channelName string) ([]entities.NotificationChannelConfig, *apperrors.Diagnostics)
}
...
func InitializeNotificationDataStore(db *gorm.DB) (NotificationDataStoreInterface, *apperrors.Diagnostics) {
	...
	if err != nil {
		diag := errors.NewDiagnostics("initialize_notification_data_store")
		diag.AddError("failed_to_initialize_notification_store", err.Error(), "notification_data_store", err)
		return nil, diag
	}
	return notificationDataStoreInstance, nil
}
...
func (s *NotificationDataStore) Migrate() error {
	if err := s.GetDB().AutoMigrate(&entities.Notification{}); err != nil {
		return fmt.Errorf("failed to migrate notification table: %s", err.Error())
	}
	if err := s.GetDB().AutoMigrate(&entities.NotificationChannel{}); err != nil {
		return fmt.Errorf("failed to migrate notification channel table: %s", err.Error())
	}
	if err := s.GetDB().AutoMigrate(&entities.NotificationChannelConfig{}); err != nil {
		return fmt.Errorf("failed to migrate notification channel config table: %s", err.Error())
	}
	return nil
}

func (s *NotificationDataStore) CreateNotification(ctx basecontext.BaseContext, tenantID string, notification *entities.Notification) (*entities.Notification, *apperrors.Diagnostics) {
	diag := errors.NewDiagnostics("create_notification")
	if notification.ID == "" {
	...
	if err := s.GetDB().WithContext(ctx).Create(notification).Error; err != nil {
...
func (s *NotificationDataStore) GetNotifications(ctx basecontext.BaseContext, tenantID string, userID string, queryObj *filters.QueryBuilder) (*filters.QueryBuilderResponse[entities.Notification], *apperrors.Diagnostics) {
	diag := errors.NewDiagnostics("get_notifications")
	db := s.GetDB().WithContext(ctx).Where("tenant_id = ? AND user_id = ?", tenantID, userID)

	result, err := filters.QueryDatabase[entities.Notification](db, tenantID, queryObj)
	if err != nil {
...
func (s *NotificationDataStore) MarkAsRead(ctx basecontext.BaseContext, tenantID string, notificationID string, userID string) *apperrors.Diagnostics {
	diag := errors.NewDiagnostics("mark_notification_as_read")
	...
	result := s.GetDB().WithContext(ctx).Model(&entities.Notification{}).
...
func (s *NotificationDataStore) MarkAllAsRead(ctx basecontext.BaseContext, tenantID string, userID string) *apperrors.Diagnostics {
	diag := errors.NewDiagnostics("mark_all_notifications_as_read")
...
func (s *NotificationDataStore) GetUnreadCount(ctx basecontext.BaseContext, tenantID string, userID string) (int64, *apperrors.Diagnostics) {
	diag := errors.NewDiagnostics("get_unread_notification_count")
	var count int64
...
func (s *NotificationDataStore) CreateChannel(ctx basecontext.BaseContext, tenantID string, channel *entities.NotificationChannel) (*entities.NotificationChannel, *apperrors.Diagnostics) {
	diag := errors.NewDiagnostics("create_notification_channel")
	if channel.ID == "" {
	...
	if err := s.GetDB().WithContext(ctx).Create(channel).Error; err != nil {
...
func (s *NotificationDataStore) GetChannelByName(ctx basecontext.BaseContext, tenantID string, name string) (*entities.NotificationChannel, *apperrors.Diagnostics) {
	diag := errors.NewDiagnostics("get_notification_channel_by_name")
	var channel entities.NotificationChannel

	result := s.GetDB().WithContext(ctx).Where("tenant_id = ? AND name = ?", tenantID, name).First(&channel)
	if result.Error != nil {
...
func (s *NotificationDataStore) GetChannels(ctx basecontext.BaseContext, tenantID string) ([]entities.NotificationChannel, *apperrors.Diagnostics) {
	diag := errors.NewDiagnostics("get_notification_channels")
	var channels []entities.NotificationChannel

	result := s.GetDB().WithContext(ctx).Where("tenant_id = ?", tenantID).Find(&channels)
	if result.Error != nil {
...
func (s *NotificationDataStore) AddConfigToChannel(ctx basecontext.BaseContext, tenantID string, config *entities.NotificationChannelConfig) (*entities.NotificationChannelConfig, *apperrors.Diagnostics) {
	diag := errors.NewDiagnostics("add_config_to_notification_channel")
	if config.ID == "" {
	...
	if err := s.GetDB().WithContext(ctx).Create(config).Error; err != nil {
...
func (s *NotificationDataStore) GetConfigsForChannel(ctx basecontext.BaseContext, tenantID string, channelName string) ([]entities.NotificationChannelConfig, *apperrors.Diagnostics) {
	diag := errors.NewDiagnostics("get_configs_for_channel")
	var configs []entities.NotificationChannelConfig
