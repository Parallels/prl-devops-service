...
func (p *PDFile) Validate() *PDFileDiagnostics {
	...
	for i, line := range p.raw {
		...
		case "RUN":
			...
			continue
		case "INSECURE":
			diag.warnings = append(diag.warnings, fmt.Errorf("insecure flag found at line %d, do not use in production", i))
			continue
		default:
	...
	if hasProvider {
		...
		if runCMD == "push" || runCMD == "pull" || runCMD == "delete" || runCMD == "import" {
