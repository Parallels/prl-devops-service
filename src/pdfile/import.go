...
func (p *PDFileService) runImport(ctx basecontext.ApiContext) (interface{}, *diagnostics.PDFileDiagnostics) {
	ctx.DisableLog()
	serviceprovider.InitServices(ctx)

	diag := diagnostics.NewPDFileDiagnostics()

	body := catalog_models.ImportCatalogManifestRequest{
	...
	host := p.pdfile.GetHostUrl() + "/import"
	var response catalog_models.ImportCatalogManifestResponse
