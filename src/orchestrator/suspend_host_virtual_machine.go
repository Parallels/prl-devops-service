...
func (s *OrchestratorService) SuspendVirtualMachine(ctx basecontext.ApiContext, vmId string, noCache bool) (*models.VirtualMachineOperationResponse, error) {
	if noCache {
	...
	result, err := s.SuspendHostVirtualMachine(ctx, vm.HostId, vmId, false)
	if err != nil {
		return nil, err
	}
...
func (s *OrchestratorService) SuspendHostVirtualMachine(ctx basecontext.ApiContext, hostId string, vmId string, useCache bool) (*models.VirtualMachineOperationResponse, error) {
	if !useCache {
		s.Refresh()
	}
	...
	result, err := s.CallSuspendHostVirtualMachine(host, vm.ID)
	if err != nil {
		return nil, err
	}
...
func (s *OrchestratorService) CallSuspendHostVirtualMachine(host *data_models.OrchestratorHost, vmId string) (*models.VirtualMachineOperationResponse, error) {
	httpClient := s.getApiClient(*host)
	httpClient.WithTimeout(3 * time.Minute)
	path := "/machines/" + vmId + "/suspend"
