...
func TestHub_Run_RegisterClient(t *testing.T) {
	...
	// Register client through command channel
...
func TestHub_Run_UnregisterClient(t *testing.T) {
	...
	emitter.hub.commands <- &unregisterClientCmd{clientID: client.ID}
	time.Sleep(50 * time.Millisecond)
...
func TestHub_Run_BroadcastMessage(t *testing.T) {
	...
	emitter.hub.commands <- &broadcastCmd{message: msg}
...
func TestHub_Run_MultipleOperations(t *testing.T) {
	...
	emitter.hub.commands <- &registerClientCmd{client: client1}
	emitter.hub.commands <- &registerClientCmd{client: client2}
	emitter.hub.commands <- &registerClientCmd{client: client3}
	time.Sleep(100 * time.Millisecond)
	...
	emitter.hub.commands <- &broadcastCmd{message: msg1}
	emitter.hub.commands <- &broadcastCmd{message: msg2}
	...
	emitter.hub.commands <- &unregisterClientCmd{clientID: client1.ID}
	time.Sleep(50 * time.Millisecond)
...
func TestHub_RegisterClient_WarningForInvalidTypes(t *testing.T) {
	...
	emitter.hub.commands <- &registerClientCmd{client: monitorClient}
	time.Sleep(50 * time.Millisecond)
