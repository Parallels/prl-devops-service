...
shutdown:      make(chan struct{}),
	...
	client := &Client{
		ID:            "test-client",
		User:          &models.ApiUser{ID: "test-client", Username: "testuser"},
		hub:           hub,
		Conn:          conn,
		Send:          make(chan *models.EventMessage, 10),
		Subscriptions: []constants.EventType{constants.EventTypeGlobal},
	...
	// Register client in hub so updateClientStateCmd can find it
	...
	// Wait for pong handler to update IsAlive via command
	time.Sleep(300 * time.Millisecond)

	// Read state from hub (client is modified by hub goroutine)
	respChan := make(chan getClientResponse, 1)
	hub.commands <- &getClientCmd{clientID: client.ID, userID: client.User.ID, response: respChan}
	resp := <-respChan
	require.NoError(t, resp.err)

	assert.True(t, resp.client.IsAlive, "IsAlive should be true after pong")
	assert.False(t, resp.client.LastPongAt.IsZero(), "LastPongAt should be updated")
}
...
shutdown:      make(chan struct{}),
	...
	// Should receive unregister command
	select {
	case cmd := <-hub.commands:
		if unregCmd, ok := cmd.(*unregisterClientCmd); ok {
			assert.Equal(t, client.ID, unregCmd.clientID)
		} else {
			t.Fatal("Expected unregisterClientCmd")
		}
	case <-time.After(1 * time.Second):
...
shutdown:      make(chan struct{}),
	...
	// Should receive unregister command when clientReader exits
	select {
	case cmd := <-hub.commands:
		if unregCmd, ok := cmd.(*unregisterClientCmd); ok {
			assert.Equal(t, client.ID, unregCmd.clientID)
