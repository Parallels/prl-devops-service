...
func TestHub_BroadcastMessage_ToType(t *testing.T) {
	...
	message := models.NewEventMessage(constants.EVENT_TYPE_PDFM, "Test PDFM message", map[string]interface{}{
		"test": "data",
	})
...
func TestHub_BroadcastMessage_ToGlobal(t *testing.T) {
	...
	message := models.NewEventMessage(constants.EVENT_TYPE_GLOBAL, "Global broadcast", map[string]interface{}{
		"broadcast": "all",
	})
...
func TestHub_BroadcastMessage_ToSpecificClient(t *testing.T) {
	hub := createTestHub()
	client1 := createTestClient("client1", "user1", []string{constants.EVENT_TYPE_PDFM})
	client2 := createTestClient("client2", "user2", []string{constants.EVENT_TYPE_PDFM})

	hub.registerClient(client1)
	hub.registerClient(client2)

	// Create a message targeting specific client
	message := models.NewEventMessage(constants.EVENT_TYPE_PDFM, "Direct message", map[string]interface{}{
		"direct": true,
	})
...
func TestHub_BroadcastMessage_ClientNotFound(t *testing.T) {
	...
	message := models.NewEventMessage(constants.EVENT_TYPE_PDFM, "Message to nobody", nil)
...
func TestHub_BroadcastMessage_ChannelFull(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		msg := models.NewEventMessage(constants.EVENT_TYPE_PDFM, "Fill message", nil)
		client.Send <- msg
	}
	...
	message := models.NewEventMessage(constants.EVENT_TYPE_PDFM, "Overflow message", nil)
...
func TestHub_BroadcastMessage_MultipleSubscribersSameType(t *testing.T) {
	...
	client2 := createTestClient("client2", "user2", []string{constants.EVENT_TYPE_VM})
	client3 := createTestClient("client3", "user3", []string{constants.EVENT_TYPE_VM})
	...
	message := models.NewEventMessage(constants.EVENT_TYPE_VM, "VM event", map[string]interface{}{
		"vm_id": "123",
	})
