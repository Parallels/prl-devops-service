...
func TestS3ChunkDownloader(t *testing.T) {
	...
	mockS3 := &MockS3Client{content: content}
	downloader := NewS3ChunkDownloader("test-bucket", "test/path", mockS3)
	...
	t.Run("Error cases", func(t *testing.T) {
		mockS3WithErrors := &MockS3Client{
			content:      content,
			headErr:      fmt.Errorf("head error"),
			getObjectErr: fmt.Errorf("get error"),
		}
		errorDownloader := NewS3ChunkDownloader("test-bucket", "test/path", mockS3WithErrors)
